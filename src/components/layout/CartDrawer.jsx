...
export default function CartDrawer() {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div className={`fixed top-0 right-0 bottom-0 w-[400px] max-w-[95vw] bg-milk z-[200] flex flex-col shadow-lift transition-transform duration-350 ${cartOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-sand">
          ...
          <button onClick={() => setCartOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-cream transition-colors">
            <X size={18} />
          </button>
        </div>
            ...
            <ul className="space-y-4">
              {items.map(item => (
                <li key={`${item.id}-${item.size}`} className="flex gap-3 pb-4 border-b border-sand last:border-0">
                  <div className="w-18 h-22 rounded-xl overflow-hidden bg-cream flex-shrink-0" style={{width:72,height:88}}>
                    <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] font-medium leading-tight mb-0.5 truncate">{item.name}</p>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 border border-sand rounded-lg overflow-hidden">
                        <button
                          onClick={() => dispatch({ type: 'UPDATE_QTY', id: item.id, size: item.size, qty: item.qty - 1 })}
                          className="w-7 h-7 flex items-center justify-center hover:bg-cream transition-colors"
                        >
                          <Minus size={12} />
                        </button>
                        <span className="text-[13px] font-medium w-5 text-center">{item.qty}</span>
                        <button
                          onClick={() => dispatch({ type: 'UPDATE_QTY', id: item.id, size: item.size, qty: item.qty + 1 })}
                          className="w-7 h-7 flex items-center justify-center hover:bg-cream transition-colors"
                        >
                  ...
                  <button
                    ...
                    className="text-muted hover:text-accent transition-colors self-start mt-0.5"
                  >
                    <Trash2 size={14} />
                  </button>
                </li>
        ...
        {items.length > 0 && (
          <div className="px-6 py-5 border-t border-sand space-y-3">
            ...
            <Link
              to="/checkout"
              onClick={() => setCartOpen(false)}
              className="btn-primary w-full justify-center text-[14px] py-4"
            >
