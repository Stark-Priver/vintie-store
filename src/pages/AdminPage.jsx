...
import { products, adminOrders } from '../data/mockData';
...
/* ── DASHBOARD tab ── */
const stats = [
  { label: 'Total Revenue',  value: '$48,295', change: '+12.5%', up: true,  icon: DollarSign,   color: 'bg-emerald-50 text-emerald-600' },
            ...
            <tbody>
              {adminOrders.slice(0, 5).map(o => (
                <tr key={o.id} className="border-b border-sand/30 last:border-0 hover:bg-milk transition-colors">
                  ...
                  <td className="px-6 py-3.5 text-muted">{o.items}</td>
                  <td className="px-6 py-3.5"><StatusBadge status={o.status} /></td>
                  <td className="px-6 py-3.5 text-muted">{o.date}</td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <tbody>
              {filtered.map(o => (
                <tr key={o.id} className="border-b border-sand/30 last:border-0 hover:bg-milk/60 transition-colors">
                  ...
                  <td className="px-5 py-4 text-muted">{o.email}</td>
                ...
                <tr key={p.id} className="border-b border-sand/30 last:border-0 hover:bg-milk/60 transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      <button className="w-7 h-7 rounded-lg border border-sand flex items-center justify-center text-muted hover:text-accent hover:border-accent transition-all">
                        <Trash2 size={12} />
                      </button>
                    </div>
                  </td>
          ...
          <tbody>
            {customers.map((c, i) => (
              <tr key={i} className="border-b border-sand/30 last:border-0 hover:bg-milk/60 transition-colors">
                ...
                <td className="px-5 py-4 text-muted">{c.orders} orders</td>
                <td className="px-5 py-4 font-semibold text-ink">${c.spent.toLocaleString()}</td>
                <td className="px-5 py-4 text-muted">{c.joined}</td>
                <td className="px-5 py-4">
